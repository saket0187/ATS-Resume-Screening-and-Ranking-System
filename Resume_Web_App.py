...
def download_nltk_data():
    ...
    try:
        ...
        try:
            ...
            st.info("The app will continue with limited text processing capabilities.")
...
def preprocess_text(text):
    """Clean and preprocess text for better analysis with fallback options"""
    try:
        ...
        # Try lemmatization, fallback to no lemmatization
        try:
        ...
        # Ultimate fallback - just clean the text
        text = text.lower()
        text = re.sub(r'[^a-zA-Z\s]', '', text)
        return text
...
def extract_keywords(text, num_keywords=50):
    ...
    try:
        ...
        vectorizer = TfidfVectorizer(max_features=num_keywords, ngram_range=(1, 2), min_df=1)
...
def keyword_matching_score(jd_keywords, resume_text):
    ...
    try:
        ...
        st.error(f"Error calculating keyword matching score: {e}")
        return 0
...
def tfidf_similarity_score(job_description, resume_text):
    ...
    try:
        ...
        st.error(f"Error calculating TF-IDF similarity: {e}")
        return 0
...
def calculate_composite_score(job_description, resume_text, resume_name):
    ...
    try:
        ...
        scores['experience_years'] = experience_years
        ...
        st.error(f"Error calculating composite score for {resume_name}: {e}")
        return {
            'composite_score': 0,
            'keyword_score': 0,
            'semantic_score': 0,
...
def generate_personalized_suggestions(job_description, resume_text, resume_name):
    ...
    try:
        ...
        st.error(f"Error generating suggestions for {resume_name}: {e}")
        return []
...
def get_enhanced_ats_scores(job_description, resumes, resume_names):
    ...
    try:
        ...
        st.error(f"Error calculating enhanced ATS scores: {e}")
        return []
...
if model is None:
    st.warning("⚠️ Sentence transformer model failed to load. Some features may have limited functionality.")
...
if uploaded_files and job_description:
    ...
    with st.spinner("Analyzing resumes using multiple algorithms..."):
        ...
        if not resumes_text:
            st.error("No text could be extracted from the uploaded PDFs. Please check the file format.")
        else:
            ...
            if enhanced_results:
                ...
                with col2:
                    ...
                    if show_detailed_scores and len(df_results) > 0:
                ...
                st.error("Failed to process resumes. Please check the files and try again.")
    ...
    with st.expander("📖 Example Usage"):
        ...
        ### Troubleshooting:
        - If you encounter NLTK errors, the app will use fallback text processing
        - If the sentence transformer model fails to load, basic scoring will still work
